...
class GitAgent:
    """
    GitAgent is a class that uses GitHub repositories to solve a given query.
    ...
    def __init__(self, init_query, use_cache=True):
        """
        Initialize the GitAgent with the given query and cache setting.
...
"""
    ...
    def run(self, funcs):
        """
        Run the GitAgent to solve the given query.
...
def main():
    """
    The main function to run the GitAgent program.
    It parses the command-line arguments, initializes the GitAgent, and runs it to solve the given query.
    """
    text = "GitAgent"
    font = "ansi_shadow"
    ...
    assistant = GitAgent(init_query, use_cache)
